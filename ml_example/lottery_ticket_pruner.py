...
I added new prune functions (stochastic, dependent) only.
"""
...
def _prune_func_stochastic(initial_weights, pretrained_weights, current_weights, current_mask, prune_percentage):
    """ Randomly prunes the weights with stochastic rounding (so will not be exact).
    This function gets called with information about the layer being pruned including the initial weights, the current
    weights, the current mask. This function should not alter these.
    This function strictly calculates the updated pruning mask to be used when the `LotteryTicketPruner` instance
    actually does the pruning.
    ...
    logger.debug('Randomly pruning weights of shape {}. Prune percentage={:.2f}%'.format(current_weights.shape,
                                                                                         prune_percentage * 100.0))
    prune_count = int(np.prod(current_mask.shape) * prune_percentage)
    nonzero_count = np.count_nonzero(current_mask)
    percentage_left_to_prune = float(prune_count) / nonzero_count
    mask = current_mask.astype(float) * (1-percentage_left_to_prune)
    new_mask = round_matrix(mask, method = "stochastic")
    return new_mask


def _prune_func_dependent(initial_weights, pretrained_weights, current_weights, current_mask, prune_percentage):
    """ Randomly prunes the weights using dependent rounding.
    This function gets called with information about the layer being pruned including the initial weights, the current
    weights, the current mask. This function should not alter these.
    This function strictly calculates the updated pruning mask to be used when the `LotteryTicketPruner` instance
    actually does the pruning.
    ...
    nonzero_count = np.count_nonzero(current_mask)
    percentage_left_to_prune = float(prune_count) / nonzero_count
    new_mask = round_matrix((1-percentage_left_to_prune) * current_mask, method = "dependent")
    return new_mask
...
class LotteryTicketPruner(object):
    ...
    def calc_prune_mask(self, model, prune_percentage, prune_strategy):
        ...
        local_prune_strats = {'random': _prune_func_random, 'smallest_weights': _prune_func_smallest_weights,
                              'stochastic': _prune_func_stochastic, 'dependent': _prune_func_dependent}
